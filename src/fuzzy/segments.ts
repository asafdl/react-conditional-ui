...
export class SegmentResolver {
    ...
    public resolve(text: string): {
        ...
        if (conjIndices.length === 0) {
            const single = this.matcher.parse(text);
            return {
                segments: [text],
        ...
        const noSplit = this.matcher.parse(text);
        ...
        for (const subset of powerSet(conjIndices)) {
            ...
            const conditions = this.parse(segments);
    ...
    private parse(segments: string[]): ParsedCondition[] {
        const conditions: ParsedCondition[] = [];
        for (const segment of segments) {
            const previous = conditions[conditions.length - 1] ?? null;
            const result = this.getInherited(segment, previous) ?? this.matcher.parse(segment);
    ...
    private getInherited(
        ...
        const full = this.matcher.parse(inheritAll);
        if (full) {
